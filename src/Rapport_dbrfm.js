...
function Accueil() {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:8081/statutrapport')
      .then((res) => {
        setStatutRapport(res.data[0].statut);
      })
      .catch((err) => console.log(err));
  ...
  const deleterapport = async (iddbrfm) => {
    try {
      await axios.delete('http://localhost:8081/delrapportdbrfm/' + iddbrfm);
  ...
  function fin(event) {
    ...
    axios.post('http://localhost:8081/fin_dbrfm')
      .then(res => {
  ...
  function nonfin(event) {
    ...
    axios.post('http://localhost:8081/nonfin_dbrfm')
      .then(res => {
        if (res.data.Status === "succes") {
  ...
  const generatePDF = () => {
    ...
    doc.save('rapport_DBRFM_' + dateHeureActuelle + '.pdf');
    ...
    <div>
      ...
      <nav className='nav'>
        ...
        <div className="navleft p-3">
          <h1 className="titre_division">DBRFM</h1>
          ...
          <Link to={`/discu_dbrfm_cordo`} className='btn btn-primary m cover mb-3'>Message</Link>
        ...
        <div className="navright">
          ...
          >
            ...
            <div className='m-2'>
              <div className='m-2'>
                <Link to={'/Ajouter_rapport_dbrfm'} className="btn btn-success ms-2">Ajouter des données</Link>
                <button
                  onClick={generatePDF}
              ...
              <table ref={tableRef} className="rounded cover bg-table">
                <thead>
                  <tr>
                    <th colSpan="5" className='center display-6'>Réalisation mensuelle {moisActuel}  {anneeActuelle} : DBRFM</th>
                  </tr>
                  <tr>
                    <th >Produit</th>
                    <th>{moisActuel}</th>
                ...
                <tbody>
                  {filteredRapport.map((data, iddbrfm) => (
                    <tr key={iddbrfm}>
                      <td>{data.produit}</td>
                      <td>{data.mois}</td>
                      <td>{data.division}</td>
                      ...
                      <td>
                        <div className='min-width-50'>
                          <Link to={`/edit_rapport_dbrfm/${data.id}`}>
                            <ModeIcon />
                          </Link>
                          <button
                            ...
                            onClick={async () => {
                              ...
                              if (result.isConfirmed) {
                                try {
                                  ...
                                  Swal.fire('Erreur', 'Une erreur est survenue lors de la suppression du rapport.', 'error');
                                }
                              }
                            }}
                          >
