...
function Accueil() {
  ...
  function fin(event) {
    ...
    axios.post('http://localhost:8081/fin_dpe')
      .then(res => {
  ...
  function nonfin(event) {
    ...
    axios.post('http://localhost:8081/nonfin_dpe')
      .then(res => {
        if (res.data.Status === "succes") {
  ...
  const generatePDF = () => {
    ...
    doc.save('rapport_DPE_' + dateHeureActuelle + '.pdf');
      ...
      <nav className='nav'>
        ...
        <div className="navright">
          ...
          >
            ...
            <div className='m-2'>
              <div className='m-2'>
                <Link to={'/Ajouter_rapport_dpe'} className="btn btn-success ms-2">Ajouter des données</Link>
                <button
                  onClick={generatePDF}
              ...
              <table ref={tableRef} className="rounded cover bg-table">
                <thead>
                  <tr>
                    <th colSpan="5" className='center display-6'>Réalisation mensuelle {moisActuel}  {anneeActuelle} : DPE</th>
                  </tr>
                  <tr>
                    <th >Produit</th>
                    <th>{moisActuel}</th>
                  ...
                  {filteredRapport.map((data, iddbrfm) => (
                    <tr key={iddbrfm}>
                      ...
                      <td>
                        ...
                        <Link to={`/edit_rapport_dpe/${data.id}`}>
                          <ModeIcon />
                        </Link>
                        <button
